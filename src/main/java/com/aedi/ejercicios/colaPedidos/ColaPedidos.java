...
public class ColaPedidos {
    ...
    public void insertarEnPosicion(String codigoPedido, int prioridad, int posicion) {
        if (posicion < 0 || posicion > size) {
            throw new IndexOutOfBoundsException("Posición inválida: " + posicion + " (tamaño actual: " + size + ")");
        }
        NodoPedido current = header;
        if (posicion == 0) {
            NodoPedido newNode = new NodoPedido(codigoPedido, prioridad, header.getNext());
            header.setNext(newNode);
        } else if (posicion == this.size) {
            NodoPedido newNode = new NodoPedido(codigoPedido, prioridad, trailer);
            ...
            NodoPedido newNode = new NodoPedido(codigoPedido, prioridad, current.getNext());
            current.setNext(newNode);
        }
        size++;
    }
